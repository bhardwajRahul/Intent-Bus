...
def test_health_check(client):
    ...
    assert response.json["version"] == "7.6"


def test_unauthorized_access(client):
    response = client.post("/intent", json={"goal": "test", "payload": {}})
...
def test_publish_and_claim_lifecycle(client):
    ...
    claim_res = client.post("/claim?goal=process_image&namespace=media", headers=headers)
    assert claim_res.status_code == 200
    assert claim_res.json["id"] == intent_id
    assert claim_res.json["payload"]["url"] == "http://example.com/image.png"

    empty_claim = client.post("/claim?goal=process_image&namespace=media", headers=headers)
    assert empty_claim.status_code == 204

    fulfill_res = client.post(
        f"/fulfill/{intent_id}",
        headers=headers,
        json={"result": {"status": "done"}, "result_type": "json"},
    )
...
def test_idempotency_keys(client):
    ...
    payload = {"goal": "test_idempotency", "payload": {"data": 1}}

    res1 = client.post("/intent", headers=headers, json=payload)
    assert res1.status_code == 201

    res2 = client.post("/intent", headers=headers, json=payload)
    assert res2.status_code == 201
    assert res1.json["id"] == res2.json["id"]

    modified_payload = {"goal": "test_idempotency", "payload": {"data": 2}}
    res3 = client.post("/intent", headers=headers, json=modified_payload)
...
def test_metrics_endpoint(client):
    ...
    res2 = client.get(
        "/metrics",
        headers={"Authorization": f"Bearer {TEST_METRICS_TOKEN}"},
    )
